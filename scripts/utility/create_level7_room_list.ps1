# Create Excel file for Level 7 rooms only
...
Write-Host "CREATING LEVEL 7 ROOM LIST EXCEL FILE" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# Filter for Level 7 rooms only - by level name "L7"
$level7Rooms = $result.rooms | Where-Object {
    $_.level -eq "L7"
}

Write-Host "Found $($level7Rooms.Count) Level 7 rooms" -ForegroundColor Green
Write-Host ""

# Prepare data for Excel using current room areas
$excelData = @()
foreach ($room in $level7Rooms) {
    # Format room number as 2 digits with leading zero
    $roomNum = "{0:D2}" -f [int]$room.number

    # Use current room area (already calculated from filled regions)
...
# Create Excel file
$excelPath = "D:\RevitMCPBridge2026\Level_7_Room_List.xlsx"
...
Write-Host "Total Level 7 rooms: $($excelData.Count)" -ForegroundColor Green
Write-Host ""
