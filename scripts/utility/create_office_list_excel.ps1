# Create Excel file with office room list
...
Write-Host "CREATING OFFICE LIST EXCEL FILE" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$offices = $result.rooms | Where-Object { $_.name -like "*OFFICE*" }
Write-Host "Found $($offices.Count) office rooms" -ForegroundColor Green
Write-Host ""
...
foreach ($office in $offices) {
    $excelData += [PSCustomObject]@{
        ...
        'Square Footage' = [math]::Round([double]$office.area, 0)
    }
}

# Sort by room number
$excelData = $excelData | Sort-Object { [int]($_..'Room Number' -replace '\D', '') }

# Create Excel file
$excelPath = "D:\RevitMCPBridge2026\Office_Floor_Plan_Areas.xlsx"
...
$worksheet.Name = "Typical Floor Plan"
...
# Add title
$worksheet.Cells.Item(1, 1) = "TYPICAL FLOOR PLAN"
...
# Add subtitle
$worksheet.Cells.Item(2, 1) = "Office Room Areas"
...
Write-Host "Total offices: $($excelData.Count)" -ForegroundColor Green
Write-Host ""
