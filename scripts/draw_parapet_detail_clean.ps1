# Clean Parapet Detail - Professional Quality
# Using BOTTOM-LEFT insertion for CMU components
# Clear annotations on left side only

param(
    [int]$ViewId = 2238350,
    [float]$BaseX = 100,    # Fresh position away from other drawings
    [float]$BaseY = 0       # Roof deck level
)

Write-Host "=== Clean Parapet Detail ===" -ForegroundColor Cyan
...
# =============================================
# SECTION 10: ANNOTATIONS (LEFT SIDE ONLY)
# =============================================
Write-Host "`n=== Annotations ===" -ForegroundColor Yellow

$textX = $roofLeft - 1.5   # All text on left side

# Coping annotation
Add-Text -Text "METAL COPING W/`rCONT. CLEAT" -X $textX -Y ($copingTop - 0.3) -LeaderX (($copingLeft + $copingRight)/2) -LeaderY ($copingTop - $copingHeight/2)

# Parapet wall
Add-Text -Text "8`" CMU PARAPET`rW/ #5 @ 48`" O.C." -X $textX -Y ($BaseY + $parapetHeight * 0.7) -LeaderX $cmuCenterX -LeaderY ($BaseY + $parapetHeight * 0.7)

# Bond beam
Add-Text -Text "BOND BEAM W/`r(2) #5 CONT." -X $textX -Y ($BaseY + $parapetHeight - 0.3) -LeaderX $cmuCenterX -LeaderY ($BaseY + $parapetHeight - $courseHeight/2)

# Counter flashing
Add-Text -Text "COUNTER FLASH.`rIN REGLET" -X $textX -Y $flashTop -LeaderX ($cmuLeft - 0.75/12) -LeaderY ($flashTop - 2/12)

# Base flashing
Add-Text -Text "BASE FLASH." -X $textX -Y ($insulTop + $flashingHeight/2) -LeaderX ($cmuLeft - 0.25/12) -LeaderY ($insulTop + $flashingHeight/2)

# Rigid insulation
Add-Text -Text "4`" RIGID INSUL." -X $textX -Y ($insulBottom + $roofInsulThick/2) -LeaderX (($roofLeft + $cmuLeft)/2) -LeaderY ($insulBottom + $roofInsulThick/2)

# Roof structure
Add-Text -Text "ROOF STRUCT.`r@ 16`" O.C." -X $textX -Y ($joistBottom + $roofStructDepth/2) -LeaderX (($roofLeft + $cmuLeft)/2) -LeaderY ($joistBottom + $roofStructDepth/2)

# Roofing membrane
Add-Text -Text "ROOFING MEMBRANE" -X $textX -Y $membraneTop -LeaderX (($roofLeft + $cmuLeft)/2) -LeaderY $membraneTop

# Stucco
Add-Text -Text "1`" STUCCO ON`rMTL. LATH" -X $textX -Y ($BaseY + 1) -LeaderX (($stuccoLeft + $stuccoRight)/2) -LeaderY ($BaseY + 1)

$pipe.Close()

Write-Host "`n=== Clean Parapet Detail Complete ===" -ForegroundColor Green
Write-Host "Location: X=$BaseX"
Write-Host "Parapet: $parapetCourses courses ($([Math]::Round($parapetHeight, 2)) ft)"
Write-Host "All annotations on LEFT side for clarity"
